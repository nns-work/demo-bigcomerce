...
export default class Cart extends PageManager {
    ...
    addCartDisclaimer() {
        ...
        const $framesProducts = $('.cart-item[data-product-id=' + 622 + ']');
        let $cartDisclaimerFrame = $('.cart-disclaimer.frame');
        let totalQuantityFrames = 0;
        if ( $framesProducts.length > 0 ) {
            $framesProducts.each(function() {
                totalQuantityFrames = totalQuantityFrames + $(this).data('quantity');
            });
            ...
            if ( $cartDisclaimerFrame.length < 1 ) {
                ...
                $cartDisclaimerFrame = $('.cart-disclaimer.frame');
            }
        }
        ...
        const $fabricPanels = $('.cart-item[data-product-id=' + 225 + ']');
        let totalQuantityFabric = 0;
        let $cartDisclaimerFabric = $('.cart-disclaimer.fabric');
        if ( $fabricPanels.length > 0 ) {
            $fabricPanels.each(function() {
                totalQuantityFabric = totalQuantityFabric + $(this).data('quantity');
            });
            ...
            if ( $cartDisclaimerFabric.length < 1 ) {
                $(`
                    ...
                    your cart to checkout.</p>
                `).insertBefore('[data-cart-totals]');
                $cartDisclaimerFabric = $('.cart-disclaimer.fabric');
            }
        }
        ...
        const $polyzorbePanels = $('.cart-item[data-product-id=' + 1184 + ']');
        let totalQuantityPolyzorbe = 0;
        let $cartDisclaimerPolyzorbe = $('.cart-disclaimer.polyzorbe');
        if ( $polyzorbePanels.length > 0 ) {
            $polyzorbePanels.each(function() {
                totalQuantityPolyzorbe = totalQuantityPolyzorbe + $(this).data('quantity');
            });
            ...
            if ( $cartDisclaimerPolyzorbe.length < 1 ) {
                $(`
                    ...
                    your cart to checkout.</p>
                `).insertBefore('[data-cart-totals]');
                $cartDisclaimerPolyzorbe = $('.cart-disclaimer.polyzorbe');
            }
        }
        ...
        if ( ($framesProducts.length > 0 && totalQuantityFrames < 3) ||
            ($fabricPanels.length > 0 && totalQuantityFabric < 5) ||
            ($polyzorbePanels.length > 0 && totalQuantityPolyzorbe < 5) ) {
            $('.cart-actions').hide();
